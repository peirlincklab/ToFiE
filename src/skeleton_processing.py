...
def find_critical_pairs(skeleton):

    cps = skeleton[0]
    cps["orig_id"] = cps.index
    cps["persistence_pair"] = cps.persistence_pair.astype(int)

    critical_index1 = cps.query("orig_id != persistence_pair").query("persistence_pair != -1").orig_id.values          # find critical points with a pair that is not themselves or -1
    critical_index2 = cps.query(f"orig_id in {tuple(critical_index1)}").persistence_pair.values                        # the corresponding pair
    
    filter = np.isin(critical_index2, cps.index)
    crit_pairs = zip(critical_index1[filter], critical_index2[filter], cps.loc[critical_index1[filter]].type.values, cps.loc[critical_index2[filter]].type.values)  # save (CP1, CP2, type CP1, type CP2) 

    pairs = []
    
    if "z" not in cps.columns:  # 2D case

        for i in list(crit_pairs):
            point1, point2 = i[0], i[1]
            type = 0

            if any(sorted([i[2],i[3]]) == sorted(item) for item in [[0, 1], [0, 3]]):   # minima-saddle
                type = 0
            elif any(sorted([i[2],i[3]]) == sorted(item) for item in [[1, 2],  [3, 2]]): # saddle-maxima
                type = 1
            else: # points are of same type, or saddle-saddle, not meant for 2D
                #print ("Same type!!!", sorted([i[2],i[3]]))
                continue
            
            value = cps.loc[[point1, point2]].field_value.values.astype(float).astype(int)
            pairs.append([type, np.min(value), abs(value[0]-value[1])])  # save eahc pair's type
    
    if "z" in cps.columns:  # 3D case
        ...
        for i in list(crit_pairs):
            ...
            if any(sorted([i[2],i[3]]) == sorted(item) for item in [[0, 1], [0, 2],[0,4]]):   # minima-saddle
                type = 0
            elif any(sorted([i[2],i[3]]) == sorted(item) for item in [[1, 2], [2, 4],[1,4]]):   # saddle-saddle
                type = 1
            elif any(sorted([i[2],i[3]]) == sorted(item) for item in [[1, 3], [2, 3], [4, 3]]):    # saddle-maxima
                type = 2
            else: # points are of same type
                #print ("Same type!!!", sorted([i[2],i[3]]))
                continue
...
def plot_persistence_diagram (skeleton, thresh):

    pairs = find_critical_pairs(skeleton)

    if "z" not in skeleton[0].columns:  # 2D case
        label = ["Type 1"]#, "Type 0"]
        types = [1]#,0]
    else:
        label = ["Type 2"]# "Type 0"]
    ...
    color_map = ["rainbow", "winter", "summer"]
    nbins=int(len(pairs)/(len(pairs)*0.01))
    print ("#bins : ", nbins)

    plt.figure(figsize=(4,3), dpi= 400)

    if "z" not in skeleton[0].columns:  # 2D case
        x = [pair[1] for pair in pairs if pair[0]== types[0]]
        y = [pair[2] for pair in pairs if pair[0]== types[0]]
        plt.hist2d(x, y, nbins, norm="log",cmap= color_map[0], label = "Type 1", alpha = 0.8)
        clb = plt.colorbar(fraction=0.05)
    
    else:

        x = [pair[1] for pair in pairs if pair[0]== types[0]]#[pair[1] for pair in pairs if pair[0]== types[0] or pair[0]== types[1]]
        y = [pair[2] for pair in pairs if pair[0]== types[0]]#[pair[2] for pair in pairs if pair[0]== types[0] or pair[0]== types[1]]
        plt.hist2d(x, y, nbins, norm="log",cmap= color_map[0], label = "Type 2", alpha = 0.8)
        clb = plt.colorbar(fraction=0.05)
    
    sigma = np.std(y)
    print (f"{thresh} sigma: {thresh*sigma}")
    plt.hlines(sigma*thresh, 0, 255, linewidth=0.5, linestyle='dashed', color = (0.2,0.2,0.2))
    plt.text(235, 1 + (sigma* thresh), f"{thresh}$\sigma$", fontsize=8)
    plt.ylabel("Persistence")
    plt.xlabel("Value")

    max_p = max([pair[2] for pair in pairs])
    plt.ylim(0,int(max_p+ 0.1*max_p))
    plt.xlim(0,255)
